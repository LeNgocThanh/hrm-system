...
export class ShiftTypesService {
  ...
  async create(dto: CreateShiftTypeDto) {
    try {
      const doc = await this.model.create({
        ...
        isCheckTwoTimes : dto.isCheckTwoTimes ?? true,
        isMixSession : dto.isMixSession ?? false,
  ...
  async findAll(params: {
    ...
    const limit = Math.min(100, Math.max(100, Number(params.limit ?? 1000)));
  ...
  async update(id: string, dto: UpdateShiftTypeDto) {
    ...
    const doc = await this.model.findByIdAndUpdate(
      id,
      {
        ...
        ...(dto.isMixSession !== undefined ? { isMixSession: dto.isMixSession } : {}),
